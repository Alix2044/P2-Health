...
io.on('connection', (socket) => {
	...
	socket.on('joinRoom', ({ roomId, userId, user }) => {
		socket.join(roomId);
		console.log(`User ${userId} named ${user} joined room: ${roomId}`);
	});

	socket.on('chatMessage', async ({ roomId, userId, message }) => {
		try {
			const user = await User.findById(userId).select('fullName');
			const newMessage = new Message({ roomId, user: userId, text: message }); //new document to mongodb
			await newMessage.save();

			io
				.to(roomId)
				.emit('message', { userId, fullName: user.fullName, message, timestamp: newMessage.timestamp }); //broadcasts the message to everyone
		} catch (err) {
